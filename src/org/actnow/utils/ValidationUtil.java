...
public class ValidationUtil {
	...
	public static void addBlueBackground(Activity activity, EditText editText) {
		...
		editText.setTextColor(activity.getResources()
	...
	public static void addGrayBackground(Activity activity, EditText editText) {
		...
		editText.setTextColor(activity.getResources()
				.getColor(R.color.app_gray));
	}

	public static boolean isEmailAddressValid(EditText editText) {
		if (editText != null && editText.getText() != null
				&& editText.getText().length() > 0) {
			return EMAIL_ADDRESS_PATTERN.matcher(editText.getText().toString())
					.matches();
		}
		return false;
	}
	...
	public static void enableButton(Activity activity, Button button,
			boolean enable) {
		if (enable) {
